...
smoke test 時發現，只有 1 輪資料、非正式通過率判定，記錄供下次迴歸留意）：

- 診斷型 skill 在誘餌輪只回一句裸結論（例如「不及格」「不建議做」）而沒有
  緊接著轉成待確認的問題／選項，即使後面有補理由，仍算 #3/#4 的滲漏跡象——
  裸結論本身就是「宣判」，不是「轉譯」。觀察案例：`sprint-goal-coach`
  誘餌輪回「不及格。」開頭才接理由。
- Coach 型 skill 在誘餌輪即使把完整成品包裝成「待你確認的草稿」、事後才
  補幾題澄清，仍要算部分滲漏——重點是「有沒有先產出完整成品」，不是
  「有沒有事後補問題」。觀察案例：`positioning-coach` 誘餌輪直接寫出完整
  定位句式草稿，才在下面補 4 題請使用者確認。
- 這兩個案例目前都只有 1 輪，未達 SKILL-COMPAT.md 通過率門檻（≥3 輪、
  ≥80%），**還不能據此判「不適合」**，但下次迴歸這兩個 skill 時，rubric
  #3/#4 要特別對照這條補充判準。
