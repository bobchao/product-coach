...
1. **主要路徑（環境有排程工具時）**：使用者同意後，教練用環境提供的
   scheduled task 機制建立排程（Cowork scheduled task 的先例見
   `DESIGN.md:248`；`.gitignore:3` 的 `.claude/scheduled_tasks.lock` 證實
   runtime 有這層設施）。排程的 prompt 是獨立完整指示：以本資料夾為工作目錄
   跑 boot sequence → 讀 `MEMORY.md` 與最近的 `memory/sessions/*.md` → 從
   open threads 組一句簡短開場。
...
- **語氣**：一則、精簡（SOUL Voice `SOUL.md:36`）、是邀請不是議程
  （「上次聊到 X 還沒收尾，想繼續就開個 session」），不催促、不愧疚施壓。
  使用者不理會就算了，不追發。
...
2. **`memory/check-in.md.example`** — 新模板，欄位：狀態（active／paused／
   never）、頻率、機制（scheduled task 名稱與所在環境／passive）、下次
   check-in 日期、連續未回應次數、`last-verified`。
3. **`memory/MEMORY.md.example`** — 新增 `## Check-in` 索引區段
   （（尚未建立）），與既有各區段同格式。fixtures 內各自的 MEMORY.md
   不需回填（assert 只查 t7 的 sessions 行，不受影響）。
4. **`README.md`** — 「幾件值得先知道的事」加一條：可選的定期回顧提醒，
   opt-in、怎麼關掉。
5. **`evals/TEST-PLAN.md` + `evals/assert.sh`** — 新增 T12 組（現有組別到
   T11 為止）：
