...
### 讓 coach 更強：裝 skills（可選）

Coach 不裝任何 skill 也完整運作。想強化特定情境（OKR 品質診斷、校準
估計、定位訪談……），可以自行安裝 product management 相關的 skills——
裝了之後不需要做任何設定，coach 會自動判斷哪些適合 coaching 使用
（準則見 `SKILLS.md`），並在合適的情境下用它們一起協助你。

推薦來源（歡迎 PR 補充）：

- [anthropics/skills](https://github.com/anthropics/skills) — Anthropic 官方 skills 集
